require 'rack'
require 'distil/request'

module Distil::RackInterface
  ...
  def call(env)
    ...
    response = begin
      request = Distil::Request.new(env)
      __router.handle_request(request)
    rescue Distil::RespondWithHTTPBasicChallenge
      respond_with_http_basic_challenge
    rescue Distil::Error => e
      respond_with_distil_error_handler(e)
    ...
    if Distil.production? || Distil.staging?
  ...
  def respond_with_http_basic_challenge
    ...
    response['WWW-Authenticate'] = %(Basic realm="Distil HTTP Basic")
    response.status = 401
  ...
  def respond_with_distil_error_handler(e)
    response = Rack::Response.new
    response.headers['Content-Type'] = 'application/json'

    if e.is_a?(Distil::RequestError)
  ...
  def stop_profiling_and_print
    ...
    printer.print(
      File.open('/Projects/distil/profile.html', 'w'),
      min_percent: 5
    )
  end
