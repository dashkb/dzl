class Distil::DSLSubjects::Parameter
  module AllowedValues
    def allowed_values(input)
      unless (@validations[:allowed_values].present? ||
        ...
        return Distil::ValueOrError.new(v: input)
      end
      ...
      if @validations[:disallowed_values].present?
        ...
        return Distil::ValueOrError.new(e: :disallowed_values_failed) unless valid
      end
      ...
      if @validations[:allowed_values].present?
        ...
        return Distil::ValueOrError.new(e: :allowed_values_failed) unless valid
      end
    ...
    def regex_match(input)
      return Distil::ValueOrError.new(v: input) unless @validations[:matches].present?
      
      error = nil

      if input.is_a?(String)
        unless @validations[:matches].any? {|re| re.match(input)}
          error = Distil::ValueOrError.new(e: :regex_match_fail)
        end
      ...
      return error ? error : Distil::ValueOrError.new(v: input)
    end
  end
end