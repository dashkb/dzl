...
class Distil::DSLSubjects::Endpoint < Distil::DSLSubject
  ...
  def handle(request)
    ...
    Distil::ResponseContext.new(self, request, @handler).__respond__
  end

  def validate(request)
    unless @opts[:request_methods].include?(request.request_method.downcase.to_sym)
      return Distil::ValueOrError.new(e: :request_method_not_supported)
    end
