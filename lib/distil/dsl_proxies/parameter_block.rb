class Distil::DSLProxies::ParameterBlock < Distil::DSLProxy
  def parameter(*names)
    ...
    names.each do |name|
      if @subject.params[name]
        ...
        @subject.params[name] = Distil::DSLSubjects::Parameter.new(name, opts)
      end
  ...
  def protect
    ...
    @subject.opts[:protection] << Distil::DSLSubjects::Protection.new
