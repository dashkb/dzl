require 'distil/validator'

class Distil::DSLProxies::Parameter < Distil::DSLProxy
  alias_method :orig_mm, :method_missing
  attr_reader :default_value
  def method_missing(m, *args, &block)
    validator = "Distil::Validators::#{m.to_s.camelize}".constantize rescue nil
    if !validator
  ...
  def respond_to?(m, *args, &block)
    ...
    ("Distil::Validators::#{m.to_s.camelize}".constantize rescue nil)
  end
